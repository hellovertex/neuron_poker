...
class PlayerCycle:
  ...
  def mark_bb(self):
    ...
    # self.last_raiser_step = self.step_counter + len(
    #  self.lst) * self.max_raising_rounds  # :custom * self.max_raising_rounds
    self.max_steps_total = self.step_counter + len(
      self.lst) * 2 * self.max_raising_rounds  # :custom * self.max_raising_rounds

  def is_raising_allowed(self):
    # todo fix using self.last_raiser_step which is only USED here, but set elsewhere
    # or self.max_steps_total and in either case using self.stack
    # todo then we can use is_raising_allowed to update on legal moves
    """Check if raising is still allowed at this position"""
    raise NotImplementedError  # :custom
    # :custom # return self.step_counter <= self.last_raiser_step
